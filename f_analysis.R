...
plot8 <- filtered_books %>%
  ggplot(aes(books_count, average_rating)) +
  stat_bin_hex(bins = 50) +
  scale_fill_distiller(palette = "Spectral") +
  stat_smooth(method = "lm", color = "orchid", size = 2) +
...
plot10 <- filtered_tmp %>%
  ggplot(aes(number_of_rated_books, mean_rating)) +
  stat_bin_hex(bins = 50) +
  scale_fill_distiller(palette = "Spectral") +
  stat_smooth(method = "lm", color = "orchid", size = 2, se = FALSE) +
...
plot13 <- books %>%
  ...
  guides(color = FALSE) +
  labs(x = "Volume of series", y = "Average rating")
ggsave("./plots/sequel_better.png", plot13)

# How long should a title be
books <- books %>%
  mutate(
    title_cleaned = str_trim(str_extract(title, "([0-9a-zA-Z]| |'|,|\\.|\\*)*")),
...
plot14 <- books %>%
  ...
  guides(color = FALSE) +
  labs(x = "Title length") +
  coord_cartesian(ylim = c(2.2, 4.7)) +
  geom_text(aes(x = ind, y = 2.25, label = n), data = tmp)
